...
3. ادخل إلى `blogger.com` بحسابك المرتبط بمدونة `yaz-blog`.
...
6. عدّل قسم **الإعدادات** أعلى الملف:
   - `SEND_URL` — مضبوط مسبقاً على لوحة الادمن المستضافة: `https://verceladmin-ten.vercel.app/api/submit`.
   - `ADMIN_EMAIL` — بريد الادمن الاحتياطي عند تعطل الـ `SEND_URL` (اختياري).
7. اضغط **نشر** (Publish) وسمّ الصفحة `serial-re` (سيصبح الرابط `.../p/serial-re.html`).
8. جرّب: افتح الرابط وأدخل سيريالاً تجريبياً — يجب أن يظهر فوراً في لوحة الادمن.

> بلوجر استضافة **ثابتة** بلا قاعدة بيانات، لذلك تُستقبل الطلبات في **لوحة الادمن (Vercel)**
> بموقع `https://verceladmin-ten.vercel.app` ثم تُحفظ تلقائياً في `admin/inbox.json`
...
## 3.7) نسخة الادمن (ADMIN) — محمية بكلمة مرور
